...
func makeCPUUtilizationMetrics(s interface{}, constLabels prometheus.Labels) []prometheus.Metric {
	...
	v := reflect.ValueOf(s)
	res := make([]prometheus.Metric, 0, t.NumField())
	desc := prometheus.NewDesc("node_cpu_average", "The percentage of CPU utilization. Units: Percent", []string{"mode"}, labels)
	for i := 0; i < t.NumField(); i++ {
		tags := t.Field(i).Tag
		mode := tags.Get("name")
...
func makeDiskIOMetrics(s interface{}, namePrefix string, constLabels prometheus.Labels, device string) []prometheus.Metric {
...
func makeFileSysMetrics(s interface{}, namePrefix string, constLabels prometheus.Labels, name, mountPoint string) []prometheus.Metric {
...
func makeNetworkMetrics(s interface{}, namePrefix string, constLabels prometheus.Labels, iface string) []prometheus.Metric {
...
func makeProcessListMetrics(s interface{}, namePrefix string, constLabels prometheus.Labels, name string, id, parentID, TGID int) []prometheus.Metric {
...
func (m *osMetrics) originalMetrics(region string) []prometheus.Metric {
	...
	metrics = makeCPUUtilizationMetrics(m.CPUUtilization, constLabels)
	res = append(res, metrics...)
